...
const StatesFriendshipSociety = () => {
  ...
  const categoryData = associationCategoryData.find(category => category.id === categoryId * 1);
  const countryData = associationData.find(country => country.id === categoryData.country_uz * 1)
  ...
  const dataHero = {
    ...
    pagePath:[
      ...
      {id:1, label:t("citizin_items.item3"), path: "/compatriots/public-associations"},
  ...
  return (
    <>
      <div className='state-friendship-society'>
        <div className="state-friendship-society__head" style={{backgroundImage: `url(https://vatanparvarbackend.napaautomotive.uz/storage/${countryData.background_flags})`}}>
          <WhriteHeader/>
          <SiteHero {...dataHero}/>
        </div>
